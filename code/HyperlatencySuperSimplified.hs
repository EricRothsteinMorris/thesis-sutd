...
solveStateDeformation::(X->F X)->(X->F X)->[M.Map X X]
solveStateDeformation start current = 
  solveHelper allTX []
    where 
      allTX = allPermutations allX --(allTransformations allX)
      solveHelper xs solutions = 
        case xs of
          [] -> solutions --brute force finished....
          (x:xs') -> --test x with fx
            let 
...
In any case, here is something to think about: imagine that the system is originally c:x->F x, and the attacker deforms the state space, yielding the system (c.s):c->F x. The question is that if there is any type of correction that we can apply, say t:F x-> F x such that (t.c.s)=c, in other words, t counteracts s. This feels a lot like resilience. alternatively, we could find s' such that (c.s'.s)=c and balance the actions of the attacker?

To solve the problem above, we need to be able to estimate the deformation. See solveStateDeformation. Well, if the state deformation is reversible then it may be possible just to reverse it as well by counteracting with another state deformation.
-}
