...
Until now, we have considered the effect of spatial transformations from an integrity perspective. We have used spatial transformations that model integrity attacks to alter the behaviour of systems, and we have quantified how robust a system is with respect to a set of attacks derived from a given attacker model. Figuratively, we say that we have so far used spatial transformations for destructive purposes. In this chapter, we explore the possibility of using spatial transformations for constructive purposes, i.e., we use them to change the behaviour of programs such that the resulting programs satisfy behavioural properties they did not before. More precisely, we are interested in enforcing protection against side-channel attacks that use the timing channel to leak secrets by means of spatial transformations. 
