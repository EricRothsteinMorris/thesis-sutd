...
The branch predication rule $\mathscr{B}\colon \mathscr{E}\rightarrow \mathscr{E}$ maps $\branch{b}{(x:=e_1)}{(x:=e_2)}$ to $x:=\textbf{sel}(b,e_1,e_2)$; this causes a change of dynamics from
\begin{align*}
...
%     \end{cases}
% \end{align*}
since the GKAT expression $1$ does not alter the current state, and we can iterate 
...
\todo[inline]{We can now transform programs, but preserving their semantics! Yahoo!} 
% \begin{align*}
%     \inferrule
