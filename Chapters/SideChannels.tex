...
\paragraph{ORIGAMI read rule.} Recall that
\begin{align*}
...
\end{align*}
The equality $\delta(x:=A[s])=\delta(\mathscr{O}(x:=A[s]))$ is not obvious at first, since we are introducing more actions during repair. We must consider the concrete semantics of actions now, just as we did when defining the equivalence $x:=\textbf{sel}(b,e_1,e_2)\equiv \branch{b}{(x:=e_1)}{(x:=e_2)}$.
...
\paragraph{ORIGAMI write rule.} Similarly to the write rule, we have
\begin{align*} 
...
\end{align*}
which is again justified since, for $\alpha \in \Atom$ and $0\leq i\leq n$,
\begin{align*}
