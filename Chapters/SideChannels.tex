...
Now that we have a dynamics function that defines how GKAT expressions evolve during computations in the forme of the $G$-coalgebra $(\mathscr{E},\delta)$, we enforce TSCF by applying a spatial transformations which implement the branch predication, loop unrolling and the read and write ORIGAMI rules. 
...
% \end{align*}
The equality $\delta(x:=A[s])=\delta(\mathscr{O}(x:=A[s]))$ is not obvious at first, since we are introducing more actions when we apply $mathscr{O}$. We must consider the concrete semantics of actions again, just as we did when defining the equivalence $x:=\textbf{sel}(b,e_1,e_2)\equiv \branch{b}{(x:=e_1)}{(x:=e_2)}$.

Let us now define the GKAT expression $\textbf{rFold}(x,A,s,i..n))$, for $0\leq i\leq n$, by
\begin{align*}
...
\begin{align*}
    x:=A[s]&\equiv \textbf{rFold}(x,A,s,0..n),
\end{align*}
which is justified since, for $\alpha \in \Atom$ and $0\leq i\leq n$, 
\begin{align*}
    \delta(\textbf{rFold}(x,A,s,i..n))(\alpha)=
    \begin{cases}
        \iota_2(x:=A[i],\textbf{rFold}(x,A,s,i+1..n)),&\quad\text{if $\alpha \Rightarrow s=i$},\\
        \iota_2(1,\textbf{rFold}(x,A,s,i+1..n)),&\quad\text{otherwise}.
    \end{cases}
\end{align*}
If $\alpha \Rightarrow s=i$, then the next action of $\textbf{rFold}(x,A,s,i..n)$ is $x:=\textbf{sel}(s=i,A[i],x)$, which is equivalent to $x:=A[i]$, which is equal to $x:=A[s]$. If $\alpha \lnot \Rightarrow s=i$, then the next action of $\textbf{rFold}(x,A,s,i..n)$ is $x:=\textbf{sel}(s=i,A[i],x)$, which is equivalent to $x:=x$. To continue, we must accept that actions of the form $v:=v$ are equivalent to 1 (i.e., they are no-ops), which we believe is a reasonable assumption. Now, by the inference rule in Equation~\ref{eq:SideChannels:EndFst}, we know that $\delta(1\cdot e)(\alpha)=\delta(e)(\alpha)$; i.e., $\delta(e)(\alpha)$ skips trivial actions. Since at most one of the conditions $\alpha \Rightarrow s=i$ is satisfied for $0\leq i\leq n$, the next action of $\textbf{rFold}(x,A,s,i..n)$ is either $x:=A[i]$ or an extreme case when $s<0$ or $s>n$, which causes $\textbf{rFold}(x,A,s,i..n)$ to terminate well.% Finally, for all $\alpha\in \Atom$, 

In the extreme case when $s<0$ or $s>n$, the semantics of $x:=A[s]$ is normally undefined; however, the semantics of $\textbf{rFold}(x,A,s,0..n)$ is to simply terminate well. Therefore, the equivalence $x:=A[s]\equiv \textbf{rFold}(x,A,s,0..n)$ is only sound with respect to functionality if the equivalence $x:=A[s]\equiv 1$ is valid when $s<0$ or $s>n$; otherwise, we face the \emph{out-of-bounds accesses} limitation from Section~\ref{sec:SideChannels:Limitations}.

\paragraph{ORIGAMI write rule.} Similarly to the read rule, to prove the equality $\delta(A[s]:=x)=\delta(\mathscr{O}(x:=A[s]))$, 
% we have
% \begin{align*} 
...
we define another GKAT expression $\textbf{wFold}(x,A,s,i..n))$, for $0\leq i\leq n$, by
\begin{align*}
    \textbf{wFold}(x,A,s,i..n)\triangleq&(\branch{(s=i)}{(A[i]:=x)}{1})\cdot \\
    &(\branch{(s=i+1)}{(A[i+1]:=x)}{1})\cdot 
    \\&
    \ldots \cdot \\
    &(\branch{s=n}{A[n]:=x}{1}).
\end{align*}
We also define the equivalence 
\begin{align*}
    A[s]:=x&\equiv \textbf{wFold}(x,A,s,0..n),
\end{align*}
that satisfies, for $\alpha \in \Atom$ and $0\leq i\leq n$,
\begin{align*}
    \delta(\textbf{wFold}(x,A,s,i..n))(\alpha)=
    \begin{cases}
        \iota_2(A[i]:=x,\textbf{wFold}(x,A,s,i+1..n)),&\quad\text{if $\alpha \Rightarrow s=i$},\\
        \iota_2(1,\textbf{wFold}(x,A,s,i+1..n)),&\quad\text{otherwise};
    \end{cases}
\end{align*}
A reasoning for soundness similar to that of the read rule follows. If $\alpha \Rightarrow s=i$, then the next action of $\textbf{wFold}(x,A,s,i..n)$ is $A[i]\textbf{sel}(s=i,x,A[i])$, which is equivalent to $A[s]:=a$. If $\alpha \lnot \Rightarrow s=i$, then the next action of $\textbf{wFold}(x,A,s,i..n)$ is $A[i]:=\textbf{sel}(s=i,x,A[i])$, which is equivalent to $A[i]:=A[i]$. Because $A[i]:=A[i]$ is equivalent to $1$, we again use the inference rule in Equation~\ref{eq:SideChannels:EndFst}, to conclude $\delta(1\cdot e)(\alpha)=\delta(e)(\alpha)$. The next action of $\textbf{wFold}(x,A,s,i..n)$ is either $A[i]:=x$ or to terminate well in the extreme case when $s<0$ or $s>n$. 
% Finally, for all $\alpha\in \Atom$, 

Again, we must accept that the dynamics of $\textbf{wFold}(x,A,s,0..n)$ is to simply terminate well in the case when $s<0$ or $s>n$; i.e., $A[s]:=x\equiv 1$ if $s<0$ or $s>n$. Otherwise, we encounter the \emph{out-of-bounds accesses} limitation from Section~\ref{sec:SideChannels:Limitations} once more.
% \begin{align*}
%     \delta(\iterate{b}{e})(\alpha)=
%     \begin{cases}
%         \iota_2(p,e'\cdot \iterate{b}{e}),&\quad\text{if $\alpha \Rightarrow b$ and $e\xrightarrow{\alpha|p}e'$}\\
%         \iota_1(1),&\quad\text{if $\alpha \Rightarrow \bar{b}$}\\
%     \end{cases}
% \end{align*}
% and
% \begin{align*}
