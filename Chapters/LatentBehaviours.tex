...
This $G$-coalgebra is not minimal. The states $(0,0)$ and $(0,1)$ are bisimilar.
...
\begin{definition}[Attack Replication]
...
The attacker $A$ wins the latency game in $\tau+1$ steps given $(w\cdot i)$ with the attack $m_{\tau+1}$ if and only if there exists an attacker $A'$ that wins the latency game in $\tau$ steps from state $x_\tau$ with the attack $m_{\tau}$\todo[inline]{COMPLETE!!!!!}
\begin{align}
...
\todo[inline]{We should do it for $\tau=1$, because it becomes clearer there...}

\begin{definition}[Forcing a Value given $w$]
