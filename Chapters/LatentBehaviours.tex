...
\begin{definition}[Finitely Supported, Consistent State Transformations]
Given an $F$-coalgebra $(X,c)$, any function of type $m\colon X\rightarrow X$ is a \emph{transformation} of $X$. A transformation $m\colon X \rightarrow X$ has \emph{finite support} iff $m(x)\neq x$ only for a finite number of $x\in X$. We denote the set of finitely supported transformations by $X^X_\omega$. A transformation $m$ is \emph{consistent} if and only if, whenever $x\sim y$, then $m(x)\sim m(y)$, for all $x,y \in X$. Henceforth, we consider only finitely supported, consistent transformations, unless explicitly mentioned otherwise.
\end{definition}
...
Given an $F$-coalgebra $(X,c)$ and a transformation $m$, the \emph{latent coalgebra of $\mathbb{X}$ under $m$}, denoted $\mathbb{X}\circ m$, is defined by
\begin{align}
    \mathbb{X}\circ m\triangleq(X,{(o\circ m, \delta\circ m })).
\end{align}
The function $\TheLatentBehaviourOfIn{\cdot}{m}{\mathbb{X}}\colon X\rightarrow \sigma F$ defines the \emph{latent behaviour map} of $\mathbb{X}$ under $m$. The homomorphism $\TheLatentBehaviourOfIn{\cdot}{m}{\mathbb{X}}$ corresponds to the semantic mapping for $\mathbb{X}\circ m$; that is, for $x\in X$, 
\begin{align}
...
Given an $F$-coalgebra $(X,c)$, we define the set of its possible attacks to be the set of endofunctions in $X$ with finite support, denoted $X^X_\omega$; a function $m\colon X \rightarrow X$ has finite support iff $m(x)\neq x$ for a finite number of $x\in X$. An attack $m$ is \emph{consistent} if and only if, whenever $x\sim y$, then $m(x)\sim m(y)$, for all $x,y \in X$. Henceforth, we consider only consistent attacks, unless explicitly mentioned otherwise.
\end{definition}
