...
\todo[inline]{Small intro to faulty systems}
...
Now, assume that the programmer is malicious, and they purposely introduce a bug which flips the bits of the current state. 
% This has several effects: the initial state which is supposed to be $(0,0)$ is now $(1,1)$ (which is accepting); if the input 1 is received, the resulting state should be $(1,) the programmer flips the bits so that now the image of $(1,1)$ on input $1$ is $(0,0)$.

% Formally, the malicious programmer implements the function $\hat{\delta}$, defined for $\vec{x}\in \vec{X}$ and $i \in 2$ by 
% \begin{align}
%     \hat{\delta}(i)(\vec{x})\triangleq (i, \lnot \vec{x}[0]).
% \end{align}
% we use a two-bit system to keep track of the current state for the implementation; the first bit corresponds to the first coordinate, and the second bit to the second coordinate. However, the first bit is faulty, and it always registers one; e.g., instead of state $(0,0)$, we obtain $(1,0)$, and instead of $(0,1)$ we obtain $(1,1)$. %This fault causes states $(0,0)$ and $(1,0)$ to collide, and similarly for states $(0,1)$ and $(1,1)$. 
Figure~\ref{fig:Transformed} shows a model of the faulty system; %the initial state is now $(1,0)$ because $m(0,0)=(1,0)$, and 
This automaton recognises the language of sequences of zeroes and ones that are either empty or end in 10; i.e., $\varepsilon+(0+1)^*10$. 
...
%         \node[state, above right  of=00] (10) {$(1,0)$};
%         \node[state, accepting, below right of=10] (11) {$(1,1)$};
%         \draw (00) edge[bend left, above] node[near end]{1} (11)
%         (00) edge[bend right, below] node{0} (01)
%         (01) edge[bend left, above] node{1} (11)
%         (01) edge[loop above] node{0} (01)
%         (10) edge[bend left, above] node{1} (11)
%         (10) edge[bend right, above] node[left, near end]{0} (01)
%         (11) edge[loop above] node{1} (11)
%         (11) edge[bend left, above] node{0} (01)
%         ;
%     \end{tikzpicture}
...
Before we explain why the automaton Figure~\ref{fig:Transformed} models the faulty system, we can test that the faulty system indeed recognises $\varepsilon+(0+1)^*10$. %$(0+1)(0+1)^*$. 
...
The initial state is thus $[(0,0),(1,0)]$. 
...
% \begin{align*}
%     %[(0,0),(1,0)]\xrightarrow{\delta(0)\times\delta(0)}
%     [(0,0),(1,0)]\xrightarrow{\delta(1)\times\delta(1)}[(1,0),(1,1)]\xrightarrow{\delta(0)\times\delta(0)}[(0,1),(1,1)];
% \end{align*}
...
The fault forces the state $(0,0)$ to behave like its image under $m$, i.e., the behaviour of $(0,0)$ in the faulty system should behave like $(1,0)$. Similarly, the state $(0,1)$ should behave $(1,1)$
...
\todo[inline]{Say in this context about revealing latent behaviours and how you are going to generalise this phenomenon to coalgebras}
...
\todo[inline]{Do we need to enforce behaviourally consistent transformations? I mean we can just focus on the case of minimal coalgebras for simplicity, but it should not be discarded right away.}
\end{definition}
