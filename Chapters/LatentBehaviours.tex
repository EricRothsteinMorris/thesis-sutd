...
The objective of this section is to explain the commutative diagram presented in Figure~\ref{fig:TheArrow}. This diagram, which we refer to as ``The Arrow of Latent-Behaviour Analysis,'' illustrates how spatial and behavioural transformations affect the behaviour of an $F$-coalgebra $(X,c)$ by changing the behaviour map from $!_c$ to $!_{b\circ c\circ m}$. 
...
\begin{figure}[h]
        ...
        \begin{tikzcd}[column sep=1.75cm, row sep=1cm]
            ...
            &
        \end{tikzcd}
        \caption{``The Arrow of Latent-Behaviour Analysis.'' This commutative diagram summarises the effect of spatial and behavioural transformations over the $F$-coalgebra $(X,c)$, respectively modelled by $m$ and $b$: the behaviour map changes from $!_c$ to $!_{b\circ c\circ m}$. Latent behaviour analysis assumes $b=\id$.}
        \label{fig:TheArrow} 
    \end{figure}
...
\begin{example}[Counter Attack]
    ...
    In the original $F$-coalgebra, all states in $2\times 2$ satisfy the property $Q$ since both $(0,0)$ and $(1,0)$ recognise the language $(0+1)^*11$, $(0,1)$ recognises the language $(0+1)^*1$, and $(1,1)$ recognises $\varepsilon + 0(0+1)^*11 + 1(0+1)^*1$. However, in the latent coalgebra revealed by the attack $m$ which swaps the coordinates of states, the state $(0,0)$ no longer satisfies $Q$, since it now recognises the language $\varepsilon+(0+1)^*10$. 
