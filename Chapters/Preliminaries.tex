...
\end{align}
The function $!_c$ is called the \emph{semantic map} because it maps $x\in X$ to its denotational semantics in $\sigma F$. For example, in the case of DFA, it maps each state in the carrier to 
...
A \emph{coalgebraic specification language} for a functor $F(X)=O\times X^I$ is an $F$-coalgebra $(\mathcal{E},c)$ where $\mathcal{E}$ is a set of expressions. The semantics of an expression $e\in\mathcal{E}$ are given by the semantic map $!_c(e)$.
...
\begin{align*}
    ...
    \delta(x=0?y:1?z\land\downarrow a)(0)&\triangleq y\\
    \delta(x=0?y\land1?z\land\downarrow a)(1)&\triangleq z.
\end{align*}

In the world of automata, the expression $x=0?x\land1?x\land\downarrow 1$ describes a single-state automaton with an accepting state which loops to itself on both inputs 0 and 1. The semantic map $!_c$ maps the expression $x=0?x\land1?x\land\downarrow 1$ to the language $2^*$.%, i.e., the language which contains every sequence in $2^*$.
