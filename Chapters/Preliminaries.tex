...
% Given a coalgebraic specification language $L=(\mathcal{E},c)$ for the functor $F$ and a notion of equivalence in $\mathcal{E}$ modelled by a function $\equiv \colon \mathcal{E}\times \mathcal{E}\rightarrow 2$, we say that $L$ is \emph{coherent} if and only if, for $e_1, e_2\in \mathcal{E}$, if $!_c(e_1) = !_c(e_2)$ then $e_1\equiv e_2$; i.e., if two expressions have the same behaviour, they must be somehow be equivalent.
