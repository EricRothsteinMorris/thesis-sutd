...
Latent behaviour analysis is a general technique for the study of extraordinary behaviours that a transition system may exhibit. 

Recall the research questions that motivate this thesis. We summarise the contributions 
...
Our intuition tells us that, given a monad $T$ with unit $\eta$ and multiplication $\mu$ that models a computational side-effect (e.g., non-determinism), and a functor $F$, the procedure in~\cite{GeneralisingDetermination} lifts an $FT$-coalgebra $(X,c)$ where $c\colon X\rightarrow F(T(X))$ to a semantically equivalent $F$-coalgebra $(T(X),c^\sharp)$ with $c^\sharp\colon T(X)\rightarrow F(T(X))$. A latent $FT$-coalgebra $(X,c\circ m)$ with $m\colon X\rightarrow X$ lifts to a semantically equivalent latent $F$-coalgebra $(T(X),c^\sharp\circ m^\flat)$ with $m^\flat\colon T(X)\rightarrow T(X)$ if $m^\flat\circ \eta=m$. The $\flat$ operator should somehow aggregate the effects of $m$ in $X$ to obtain the transformation $m^\flat$; e.g., in the case of non-determinism, if $X=\set{a,b}$ and $m\colon X\rightarrow X$ maps $a$ to $b$, and $b$ to itself, the transformation $m^\flat\colon \ThePowersetOf{X}\rightarrow \ThePowersetOf{X}$ maps $\emptyset$ to itself, $\set{a}$ to $\set{b}$, $\set{b}$ to itself, and $\set{a,b}$ to $\set{b}$. We do not yet know the properties of the $\flat$ operator that are required to ensure the existence of the lifted latent $F$-coalgebra, and we leave them for future work. 
