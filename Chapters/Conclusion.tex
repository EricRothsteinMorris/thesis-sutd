%!TEX root = ../main.tex
\chapter{Conclusion}
LBA is a technique to study the unusual behaviours that a transition system may exhibit in the presence of an external force, which may be due to an attacker or a fault. In summary, LBA proposes to model the transition system as an $F$-coalgebra $(X,c)$ and the external force as an endofunction $m\colon X\rightarrow X$; we then study the effects that $m$ has on $(X,c)$ by working in the latent $F$-coalgebra $(X,c\circ m)$. Although the idea is quite simple, it has a variety of useful practical applications. For example, to model an attacker $\alpha$ of an $F$-coalgebra $(X,c)$, we use a set of spatial transformations; i.e., $\alpha$ is a subset of the set $X\rightarrow X$ of endofunctions in $X$. 

Due to its focus on spatial transformations, LBA has limitations in its applicability. For example, we cannot capture the attackers of a system $(X,c)$ that can directly change the program $c$. Nevertheless, LBA is expressive enough to model man-in-the-middle attackers, and attackers that can override the values of state variables while leaving the program of the transition system intact. 

We use the Research Questions ~\ref{que:AttackerModel},~\ref{que:Quantification}, ~\ref{que:Classification} and ~\ref{que:Repair} to provide an intuition of the usefulness of LBA in the field of systems security. We studied these questions in three practical scenarios: the classification of attacker models in Chapter~\ref{ch:Classification}, the quantification of robustness in CPS in Chapter~\ref{ch:CPSRobustness}, and timing side-channel repair in Chapter~\ref{ch:SideChannelRepair}. Although these three applications are quite different in terms of the domain of their problems, we successfully approached them using LBA, attesting the applicability and generality of the method. 
...
\subsection{Beyond Deterministic Transition Systems}
