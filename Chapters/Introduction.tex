...
Since $(\Real\times\Real, \delta)$ is the current transition system, we interpret $m$ as a spatial transformation whose application reveals the system shown in Figure~\ref{fig:SecondLatent}%, where we apply $m$ first and then $\delta$, mapping $(x,y)$ to $(y^2,x)$. 
...
% The transformation $m$ causes the behaviour of the system to lose the behavioural property of preserving the value of $y$ for every pair $(x,y)$.  so if an attacker was capable of applying $m$ to the system $(\Real\times\Real, \delta)$, then they can clearly break the value preservation property. 
