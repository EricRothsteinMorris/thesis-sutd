...
\emph{Latent behaviour analysis} (LBA) is the study of the changes to observable behaviour in a transition system when an external force transforms the state space. These spatial transformations may occur because of, for example, an attack or a component fault. LBA uses the framework of universal coalgebra to seamlessly combine the effects of a spatial transformation with the operational semantics of a transition systems; this combination results in a so-called latent coalgebra that models a new transition system. This resulting transition system is compatible with the usual verification and testing methods, and its analysis returns properties of the original system in the presence of faults or adversaries. 
